...
def what_time(friend):
    ...
    time_formated = utc_time + dt.timedelta(hours=UTC_OFFSET[city])
    return time_formated.strftime('%H:%M')
    ...
    if name in DATABASE:
        ...
        if query == 'ты где?':
            ...
            if city in UTC_OFFSET:
                time_at_friand = what_time(city)
                return f'Там сейчас {time_at_friand}'
